...
export default function ProductResult({ data, modelType }: ProductResultProps) {
        ...
        return (
          <div className="space-y-4 animate-slide-in-right">
            {[
              ...
              <div 
                key={index}
                className={`flex justify-between items-start p-4 rounded-xl transition-all duration-200 hover:bg-gray-50 ${
        ...
        return (
          <div className="space-y-4 animate-slide-in-right">
            {[
              ...
              <div 
                key={index}
                className="flex justify-between items-center p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-all duration-200"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 relative overflow-hidden">
      ...
      <div className={`relative z-10 w-full max-w-7xl mx-auto p-4 md:p-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        
        {/* Success Header */}
        <div className="text-center mb-8 animate-scale-in">
          <div className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-full shadow-lg shadow-emerald-500/30 mb-4">
            <CheckBadgeIcon className="h-6 w-6" />
            <span className="font-bold text-lg">Produk Asli Terverifikasi</span>
            <SparklesIcon className="h-5 w-5 animate-pulse" />
          </div>
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-emerald-800 via-slate-800 to-emerald-700 bg-clip-text text-transparent mb-2">
            {data.product_name}
          </h1>
          <p className="text-xl text-slate-600 font-medium">{data.varietas}</p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-8">
              <div className="relative group">
                <div className="aspect-square rounded-3xl overflow-hidden bg-white shadow-2xl border border-white p-6 transition-all duration-300 hover:shadow-3xl hover:scale-105">
                  {data.product_photo ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl">
                      <QrCodeIcon className="w-20 h-20 mb-4 opacity-50" />
                ...
                <div className="mt-6 grid grid-cols-3 gap-3">
                  <div className="text-center p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-white shadow-lg">
                    <ShieldCheckIcon className="w-8 h-8 text-emerald-500 mx-auto mb-2" />
                    <div className="text-xs font-bold text-emerald-700">ASLI</div>
                  </div>
                  <div className="text-center p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-white shadow-lg">
                    <CheckBadgeIcon className="w-8 h-8 text-blue-500 mx-auto mb-2" />
          ...
          <div className="lg:col-span-3">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/80 overflow-hidden">
                ...
                <nav className="flex" aria-label="Tabs">
                  {tabConfig.map((tab, index) => (
                    <button
                      ...
                      className={`flex-1 group relative px-6 py-6 text-sm font-semibold transition-all duration-300 ${
                        activeTab === tab.id
                    ...
                    >
                      ...
                      {activeTab === tab.id && (
                        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-12 h-1 bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-full"></div>
                      )}
                    </button>
                  ))}
                </nav>
              ...
              {modelType === 'production' && data.qr_code_link && (
                <div className="border-t border-gray-100 p-8 bg-gradient-to-r from-gray-50/50 to-white">
                  <a
                    href={data.qr_code_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative flex items-center justify-center w-full gap-3 px-8 py-4 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-bold rounded-2xl hover:from-emerald-500 hover:to-emerald-400 transition-all duration-300 shadow-xl shadow-emerald-500/30 hover:shadow-2xl hover:shadow-emerald-500/40 hover:scale-105 overflow-hidden"
                  >
                    {/* Button Shine Effect */}
                    ...
                    <div className="relative z-10 flex items-center gap-3">
                      ...
                      <span className="text-lg">Lihat Sertifikat Resmi di Kementan</span>
                      <ArrowTopRightOnSquareIcon className="h-5 w-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-200" />
                    </div>
                  </a>
                  
                  <p className="text-center text-slate-500 text-sm mt-4 flex items-center justify-center gap-2">
                    <ShieldCheckIcon className="w-4 h-4 text-emerald-500" />
                    Dokumen resmi dari Kementerian Pertanian RI
                  </p>
                </div>
              )}
            </div>
