...
export default function VarietyForm({ isOpen, onClose, varietyToEdit }: VarietyFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name) {
      toast.error("Nama Varietas wajib diisi.");
      return;
    }
    setIsSubmitting(true);
    
    const actionPromise = isEditMode && varietyToEdit
      ? updateVariety(varietyToEdit.id, formData)
