// app/admin/jenis-tanaman/PlantTypeForm.tsx
'use client';
...
import { createPlantType, updatePlantType, PlantTypeFormData } from './actions';
...
interface PlantTypeFormProps {
  isOpen: boolean;
  onClose: () => void;
  plantTypeToEdit?: PlantType | null;
}

export default function PlantTypeForm({ isOpen, onClose, plantTypeToEdit }: PlantTypeFormProps) {
  const [formData, setFormData] = useState<PlantTypeFormData>({ name: '', description: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const isEditMode = !!plantTypeToEdit;

  useEffect(() => {
    if (isOpen) {
      if (isEditMode && plantTypeToEdit) {
        setFormData({
          name: plantTypeToEdit.name,
          description: plantTypeToEdit.description || '', // Pastikan deskripsi tidak null
        });
      } else {
        setFormData({ name: '', description: '' });
      }
    }
  }, [isOpen, plantTypeToEdit, isEditMode]);
