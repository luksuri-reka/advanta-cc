...
export default function PlantTypeClient({ initialPlantTypes }: PlantTypeClientProps) {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      const promise = deletePlantType(id);
      ...
      <main className="mx-auto max-w-7xl py-8 px-4 sm:px-6 lg:px-8">
        <div className="md:flex md:items-center md:justify-between pb-6 border-b border-gray-200">
          <div className="min-w-0 flex-1">
            <h1 className="text-3xl font-bold text-gray-900">Data Jenis Tanaman</h1>
            <p className="mt-1 text-md text-gray-600">Kelola semua jenis tanaman yang terdaftar dalam sistem.</p>
          </div>
              ...
              <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                <table className="min-w-full divide-y divide-gray-300">
                  <thead className="bg-gray-50">
                    <tr>
                      <th scope="col" className="px-6 py-3.5 text-left text-sm font-semibold text-gray-900">Nama Jenis Tanaman</th>
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {initialPlantTypes.length > 0 ? initialPlantTypes.map((plantType) => (
                      <tr key={plantType.id} className="even:bg-gray-50">
                        <td className="whitespace-nowrap px-6 py-4 text-sm font-semibold text-gray-900">{plantType.name}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 max-w-sm truncate" title={plantType.description}>{plantType.description || '-'}</td>
                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{formatDate(plantType.created_at)}</td>
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <button onClick={() => handleEdit(plantType)} className="text-emerald-600 hover:text-emerald-900 inline-flex items-center gap-1">
                            <PencilIcon className="h-4 w-4" /> Edit
                          </button>
                          <button onClick={() => handleDelete(plantType.id)} className="text-red-600 hover:text-red-900 ml-4 inline-flex items-center gap-1">
                      ...
                      <tr>
                        <td colSpan={4} className="text-center py-16 px-6 text-gray-500">
                          ...
                          <p className="mt-1 text-sm">Mulai dengan menambahkan data jenis tanaman baru.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
